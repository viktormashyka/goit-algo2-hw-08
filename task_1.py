...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        if user_id not in self.user_windows:
            return
        # Видаляємо застарілі повідомлення
        while self.user_windows[user_id] and current_time - self.user_windows[user_id][0] > self.window_size:
            self.user_windows[user_id].popleft()
        # Якщо вікно порожнє, видаляємо запис про користувача
